...
def test_psoc_compounds_with_time() -> None:
    ...
    state = CellState(soc=0.7, health=1.0, cumulative_throughput_ah=0,
                      time_in_psoc_hours=0, arrhenius_age_factor=0,
                      sulfation_index=0)
    short = update_health(state, dt_hours=24, ambient_temp_c=25,
                          current_a=0, nominal_capacity_ah=200)
    long = update_health(state, dt_hours=24*30, ambient_temp_c=25,
                         current_a=0, nominal_capacity_ah=200)
...
def test_coulomb_discharge_lowers_soc() -> None:
    ...
    new = coulomb_counting_step(state, current_a=10.0, dt_hours=1.0,
...
def test_coulomb_clips_to_unit_interval() -> None:
    state = CellState.fresh()
    state.soc = 0.99
    new = coulomb_counting_step(state, current_a=-100.0, dt_hours=10.0,
                                nominal_capacity_ah=200.0)
    assert new.soc <= 1.0
    new2 = coulomb_counting_step(state, current_a=100.0, dt_hours=100.0,
                                 nominal_capacity_ah=200.0)
    assert new2.soc >= 0.0
...
def test_coulomb_capacity_fade_reduces_effective_capacity() -> None:
    ...
    degraded = CellState(soc=1.0, health=0.5, cumulative_throughput_ah=0,
                         time_in_psoc_hours=0, arrhenius_age_factor=0,
                         sulfation_index=0)
    h_after = coulomb_counting_step(healthy, current_a=20, dt_hours=5,
                                    nominal_capacity_ah=200.0)
    d_after = coulomb_counting_step(degraded, current_a=20, dt_hours=5,
                                    nominal_capacity_ah=200.0)
...
def test_health_monotonically_decreases() -> None:
    ...
    new = update_health(state, dt_hours=24, ambient_temp_c=30,
                        current_a=0, nominal_capacity_ah=200.0)
    assert new.health <= state.health


def test_health_decays_faster_at_high_temp() -> None:
    state = CellState.fresh()
    cool = update_health(state, dt_hours=24*30, ambient_temp_c=20,
                         current_a=0, nominal_capacity_ah=200.0)
    hot = update_health(state, dt_hours=24*30, ambient_temp_c=45,
                        current_a=0, nominal_capacity_ah=200.0)
...
def test_health_decays_faster_in_psoc() -> None:
    ...
    psoc = CellState(soc=0.7, health=1.0, cumulative_throughput_ah=0,
                     time_in_psoc_hours=0, arrhenius_age_factor=0,
                     sulfation_index=0)
    full_after = update_health(full, dt_hours=24*30, ambient_temp_c=25,
                               current_a=0, nominal_capacity_ah=200.0)
    psoc_after = update_health(psoc, dt_hours=24*30, ambient_temp_c=25,
                               current_a=0, nominal_capacity_ah=200.0)
...
def test_open_circuit_voltage_full_charge() -> None:
    state = CellState.fresh()
    v = shepherd_discharge_voltage(state, current_a=0, nominal_capacity_ah=200, n_cells=24)
...
def test_voltage_drops_under_load() -> None:
    state = CellState.fresh()
    v_idle = shepherd_discharge_voltage(state, current_a=0, nominal_capacity_ah=200, n_cells=24)
    v_load = shepherd_discharge_voltage(state, current_a=30, nominal_capacity_ah=200, n_cells=24)
    assert v_load < v_idle


def test_voltage_falls_with_soc() -> None:
    high_soc = CellState.fresh()
    low_soc = CellState(soc=0.2, health=1.0, cumulative_throughput_ah=0,
                        time_in_psoc_hours=0, arrhenius_age_factor=0,
                        sulfation_index=0)
    v_high = shepherd_discharge_voltage(high_soc, current_a=20, nominal_capacity_ah=200, n_cells=24)
    v_low = shepherd_discharge_voltage(low_soc, current_a=20, nominal_capacity_ah=200, n_cells=24)
    assert v_low < v_high
...
def test_sulfation_steepens_drop() -> None:
    ...
    healthy = CellState.fresh()
    sulfated = CellState(soc=1.0, health=0.9, cumulative_throughput_ah=0,
                         time_in_psoc_hours=500, arrhenius_age_factor=0,
                         sulfation_index=2.0)
    v_h = shepherd_discharge_voltage(healthy, current_a=30, nominal_capacity_ah=200, n_cells=24)
    v_s = shepherd_discharge_voltage(sulfated, current_a=30, nominal_capacity_ah=200, n_cells=24)
    assert v_s < v_h
...
def test_charge_taper_near_full() -> None:
    ...
    state = CellState.fresh()
    state.soc = 0.95
    accepted = charge_acceptance_current(state, available_charger_current_a=20,
                                         nominal_capacity_ah=200)
    assert accepted < 20.0


def test_low_soc_accepts_full() -> None:
    state = CellState(soc=0.5, health=1.0, cumulative_throughput_ah=0,
                      time_in_psoc_hours=0, arrhenius_age_factor=0,
                      sulfation_index=0)
    accepted = charge_acceptance_current(state, available_charger_current_a=20,
                                         nominal_capacity_ah=200)
...
def test_degraded_battery_accepts_less() -> None:
    ...
    healthy = CellState(soc=0.5, health=1.0, cumulative_throughput_ah=0,
                        time_in_psoc_hours=0, arrhenius_age_factor=0,
                        sulfation_index=0)
    degraded = CellState(soc=0.5, health=0.7, cumulative_throughput_ah=0,
                         time_in_psoc_hours=500, arrhenius_age_factor=0,
                         sulfation_index=2.0)
    a_h = charge_acceptance_current(healthy, available_charger_current_a=20,
                                    nominal_capacity_ah=200)
    a_d = charge_acceptance_current(degraded, available_charger_current_a=20,
                                    nominal_capacity_ah=200)
    assert a_d < a_h
